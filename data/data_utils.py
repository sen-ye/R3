...
def pad_resize_image(img, size=(256, 256)):
    width, height = size
    long_side = max(img.width, img.height)
    img_pad = Image.new('RGB', (long_side, long_side), (255, 255, 255))
    img_pad.paste(img, (0, 0))
    img = img_pad.resize((width, height))
    return img


def load_pad_resize_image(img_path, size=(256, 256)):
    img = Image.open(img_path).convert('RGB')
    return pad_resize_image(img, size)


def resize_img(img, short_side=None, long_side=None):
    # resize img so that its shortest side is short_side and longest side is long_side, and keep the aspect ratio
...
def load_resize_image(img_path, short_side=None, long_side=None):
    img = Image.open(img_path).convert('RGB')
    return resize_img(img, short_side, long_side)


def resize_img_by_wh(img, width, height, multiplier=44):
    width = int(width * multiplier)
    height = int(height * multiplier)
    return img.resize((width, height))


def load_resize_img_by_wh(img_path, width, height, multiplier=44):
    img = Image.open(img_path).convert('RGB')
    return resize_img_by_wh(img, width, height, multiplier)