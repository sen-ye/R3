...
class InterleaveInferencer:
    def __init__(self, model:Bagel, vae_model, tokenizer, vae_transform, vit_transform, new_token_ids):
        self.model = model
    ...
    def decode_image(self, latent, image_shape, batch_size: int = 1):
        ...
        for i in range(batch_size):
            images.append(Image.fromarray((image[i]).to(torch.uint8).cpu().numpy()))
        
        if batch_size == 1: # keep the same return type as single image
            images = images[0]
        return images
        ...
        with torch.autocast(device_type="cuda", enabled=True, dtype=torch.bfloat16):
            ...
            for input_term in input_lists:
                if isinstance(input_term, str):
                    ...
                    gen_context = self.update_context_image(input_term, gen_context, vae=force_vae)
            ...
            if understanding_output:
                ...
                img = self.gen_image(
                    image_shapes, 
                    gen_context, 
                    cfg_text_precontext=cfg_text_context, # without instruction text
                    cfg_img_precontext=cfg_img_context, # without to be edited image
